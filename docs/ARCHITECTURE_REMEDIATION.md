...
| T3.1 Space core | ▶ in progress — design SETTLED | Decision (user-ratified): chain-as-rewrite-unit stays — it is the better architecture, not a compromise (demand analysis and the map-terminal invariant are chain-scoped; per-node resolution conserves that complexity into side channels: V1's identity sidecar/restamp family, per-relop join scoping, lowering fallbacks; the real engine needs a merge pass). The greenfield form, correctly translated: NOT a stored stamp (would recreate the T2.2 re-founded-state class and leak H-semantics into G; space is a pure function of variant + checked types in info()) but ONE exhaustive sealed-switch classifier in the resolver (compile-breaks on new variants like 68 overrides would), memoized for O(n), feeding a TWO-LEVEL resolveNode (space first, variant second — guard ORDER becomes structure), default arm deleted (INERT = explicit identity; missing ANCHORED arm = named wall), isObjectSpace + scattered predicates deleted. The audit's goal accepted; its stamp-from-Application.chosen() mechanism rejected for the layering/drift reasons above. Full bottom-up anchor-driven inversion = explicit NON-GOAL. |
| T3.2 / T4 | pending | |
