...
| T3.1 Space core | pending — DESIGN FIRST | Open decisions before code: (1) where the stamp lives — a mandatory `Space space()` member is O(tree-height) per query unless cached; a stored field on 68 records touches every construction site; an `ExprType` component touches hundreds. The audit's "set by the checker from `Application.chosen()`" implies construction-time knowledge — the likely shape is a per-variant O(1) rule over children's spaces, computed once in canonical constructors. (2) `withChildren`/resolver rebuilds must RE-derive the stamp when a child flips OBJECT→RELATION (the resolver's own rewrites do this constantly) — the stamp must be a function of (variant, children), never free-floating state. (3) The two `containsGetAll` impls disagree (ClassSources:260 skips `TypedNavigate` internals — resolver-output vocabulary; StoreResolver:1203 descends everything): the Space design must say which reach is correct per consumer, not merge them blindly. (4) Determinism rider: `InnerDemand` `inQueryReads` IdentityHashMap iteration order (the T1.9/ResolveNestedTemporalFrameTest bistability) — ordered structures or an ordering pass; then tighten the T1.9 bistable pin to rows-only. |
| T3.2 / T4 | pending | |
