...
> **Correction on merge to main (2026-09-15, the session that owns the arc):** this section
> describes the AUDITOR'S environment, not the repository's. The auditor read the checkouts under
> `/Users/neemsandv` (4.137 / 5.92, stale since August) and ran maven by hand. The repository's
> roots resolve through `tools/oracle-roots.sh` to `$HOME/legend/legend-engine`, which is at
> `230c159196d` = `legend-engine-4.145.0`, the pinned commit; the corpus lanes on the day of the
> audit reported `declared=2761 excluded=148 discovered=2613` — the acceptance test P0-0 asks for —
> and DuckDB 108 / H2 444 EXACT. The corpus gate is not opt-in: it is gates 4 and 5 of every
> `tools/allgates.sh` chain and runs in CI on three platforms. So "EXACT is unreproducible" and
> "the row judge cannot run" below are false; the verdicts that lean on them ("bulletproof: no")
> should be read as: correct on every row the corpus contains, with real bugs (§4) in shapes the
> corpus does not contain. P0-0 in the FIXLIST is therefore a no-op here. Everything else in the
> audit was checked against the code by the owning session and stands.
