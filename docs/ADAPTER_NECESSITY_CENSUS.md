...
| B3 | Decoration strip regexes in `sliceFunction` (`<<...>>`, `{doc…}`) | Verbatim injection: our parser already parses stereotypes/tagged values (the parity program); the question is compile-side profile resolution. If the PCT profile registers as a known platform profile (it IS real upstream pure), sliced source injects UNMODIFIED. Fallback: a string-literal-aware char walk (no regex, no corruption class). | **EXECUTED 2026-08-27** — probed first: the compiler accepts stereotypes and tagged values on injected functions VERBATIM (bare `<<PCT.test>>`, the fully-qualified spelling, and `{doc.doc=...}` all compile and execute). Both decoration-strip regexes DELETED; sliced source injects byte-verbatim. Full lane 1115/0. |
...
| B8 | P7 Decimal→Float relabel (1 witness) | Real pure's Float literals ARE decimal-exact (testBigFloatAbs asserts the digits): the truthful fix is label fidelity — the typer labels the literal Float while the carrier stays decimal-exact, the census registers the DECIMAL←Float pair, the adapter keys on the PLATFORM label. The relabel arm then dies. | **EXECUTED 2026-08-27 (user P0)** — proven from the reference SOURCE first: legend-pure's interpreted Float IS BigDecimal-backed by declaration (`FloatCoreInstance extends PrimitiveCoreInstance<BigDecimal>`; `newFloatCoreInstance(String)` parses source text; the Abs native computes `BigDecimal.abs()` and re-wraps Float). Our deviation was PARSE-TIME: the species change to CDecimal made the TYPE lie to keep the VALUE. Landed: `CFloat`/`TypedCFloat` carry optional exact digits (Float label end to end; the engine byte-parity surface stays bare double, mirroring their wire record); the lowering emits the same exact DecimalLit; the equality fold compares exactly; the bridge accepts BigDecimal-under-FLOAT as the reference's own Float shape; **P7 — the adapter's last declared-type read — DELETED**. The honest label FLUSHED TWO LATENT BUGS, both fixed at their owners with probes: (1) the verdict layer's carrier-vs-kind conflation (a runtime BigDecimal read as pure-Decimal; now static-truth-gated in selectedFineKind), (2) the sub-1 DecimalLit fact under-stating precision by one (DuckDB counts the leading zero — probed `0.99 -> DECIMAL(3,2)`; the corr-literal wire divergence's root). Both PCT lanes 1115/0; full chain green. NOTE: Lowerer.java now sits AT its 3500-line hard guardrail — the prescribed seam-split precedes any future work there. |
| B9 | `substituteOpenVariables` "not capture-safe" (audit C21) | PROOF, not code: the substitution replaces variables with CLOSED VALUES (captured lambdas are closed terms) — closed-term substitution cannot capture. Written argument lands in the file; the nitpick dies by proof or, if the proof fails on a witness, by α-freshening. | **UPGRADED 2026-08-27 (user-directed): closed by construction + executable wall.** The proof's thin spot was captured lambda VALUES: a closure's environment exists in the interpreter's object world but the TEXT transport erases environments — so the substitution now resolves a captured lambda's OWN environment recursively BEFORE splicing (closed as text by construction), and `assertClosed` enforces the free-variable postcondition on every test (binder-scoped walk covering exactly the substitution's own node vocabulary, let-bindings threaded). Measured: ZERO wall firings across the full lane — the premise holds mechanically, not rhetorically. Alpha-renaming deliberately rejected (the cure for substituting OPEN terms, which we never do; it would perturb printed parameter names that tests assert). |
...
| B11 | Class extraction degrades type parameters to Any | Bounded honestly: emit the parameter list when the source class is generic (extraction reads M3 typeParameters), or wall LOUDLY on generic user classes instead of degrading silently. | **EXECUTED 2026-08-27** — the generic-class degrade and the no-packageable-path property drop are LOUD WALLS now (zero firings on the full lane — no witness was being silently tolerated); the metamodel-typed-property skip remains as the ONE documented skip (a test using such a property fails loudly at its own compile). |
...
**Gap A — the design decision, recorded (2026-08-27 adjudication with
the user):** text is a SOLVED problem for this codebase today (the
parser is byte-parity-oracle-hardened; the transport is verbatim and
walled), so the structural handoff is not purity work — its price tag
is the ~20 test rows text can NEVER carry: reference identity (the 14
"value-serializing wire" permanent exclusions — permanent only while
the wire serializes) and source positions (the 6 error-position rows).
WHEN done: map their M3 to OUR PROTOCOL layer (CFloat/AppliedFunction —
near-cousins of their metamodel), NEVER a second M3→HIR front door (a
second ingestion path re-implements every parser-side decision — the
B8 literal-typing class — and can drift: the third-impl disease).
Verification = the R1 method one level deeper: run text and structural
side by side over the full lane and require PROTOCOL-TREE EQUALITY,
not just outcome equality. Endgame: ModelPacker + the print step
delete wholesale; ValueBridge and the entry untouched. Sequence when
the PCT burndown's remainder is dominated by the identity/position
families.

### Campaign close (2026-08-27) — the hostile-review pass

Final fresh read of all four files plus a mechanical sweep. What a
hostile Pure reviewer finds today: ONE declared-type read in the pure
adapter (P7 — value-preserving, label-only, one witness, chartered to
T4 with the mechanism sized); ONE name test (`::tests::` — upstream's
own fixture-package taxonomy, walled against shadowing); ONE env knob
(the backend selector); `default` arms that are type-shape dispatch or
precision-total mappings, never value fallbacks; and walls everywhere
else. Every surviving line is plumbing, a wall, or carries its receipt.
The two structures below this floor — the Gap A transport and the
error-POSITION channel — are chartered architecture, not compensation.
End state: PctExecuteNative 131 / ModelPacker 266 / ValueBridge ~350
stripped lines + pct_adapter.pure 408 (collector included), every pin
at measure, both PCT lanes and the full chain green on every batch.
