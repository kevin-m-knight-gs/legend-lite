...
| B8 | P7 Decimal→Float relabel (1 witness) | Real pure's Float literals ARE decimal-exact (testBigFloatAbs asserts the digits): the truthful fix is label fidelity — the typer labels the literal Float while the carrier stays decimal-exact, the census registers the DECIMAL←Float pair, the adapter keys on the PLATFORM label. The relabel arm then dies. | **CHARTERED TO T4 2026-08-27, with the mechanism sized**: the promotion happens at PARSE TIME — a precision-losing float literal changes SPECIES to CDecimal (SpecParser.parseFloat, oracle-verified), so the label/carrier split needs a float-born decimal node whose tendrils reach print forms (no D suffix), every Float consumer, the census delivers-pair, and comparators — the T4 label-at-construction slice in miniature. The surviving arm is VALUE-PRESERVING and label-only (the platform delivers the decimal-EXACT digits real pure itself asserts; the label moves TOWARD real pure's own Float labeling), one witness, receipt in-file. The last declared-type read in the pure adapter, and it is an assertion-shaped relabel, not a decision. |
