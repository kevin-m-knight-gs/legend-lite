...
- **TDS join let-bound JoinType LANDED** (`JoinChecker.resolveLetBoundArgs`): +23 flips, **ratchet 848 fallbacks / 1725 flipped**. Residue 2 = let-bound `{a:TDSRow[1],b:TDSRow[1]|…}` condition (deferred-kind candidate, bind-once charter).
- **TDG ARM REACH LANDED (zero net flips, honest movement).** STEP 4 below was a misdiagnosis: the carrier is already folded to a `TestDataGenResult` literal per statement; the verdict layer just did not recognize the folded form, and `sqlRemoveFormatting` on the actual side claimed it as an exec-read. Now the 29-test cohort reaches the TDG fetch-text arm: 12 decline BY NAME "chained fetch — generator temp tables not replayable" (text contract, dialect text differs → counted fail), 7 join plan-execute values-binding; "no scalar lowering" 66 → 51. Next for TDG: replay the generator's temp tables on the oracle (an oracle leg). Then: dialect-loop unroll (13), plan-execute values-binding (17).
