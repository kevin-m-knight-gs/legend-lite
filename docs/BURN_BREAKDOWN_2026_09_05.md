...
Status: **batch 115 / harness rebuild step B LANDED (2026-09-06)** — the old harness is deleted (13.6k lines: the walk, the old runner + driver, the flip machinery, the forms); gate 4 = MinimalCorpusTest (DuckDB floor 2454), gate 5 = the same under -Drcorpus.backend=h2 (floor 1866 = the old lane's PLATFORM roster 1855 + 11; its reported 1976 counted 121 walk answers). One mechanism for helper programs (StatementInline; the executor's call-frame route deleted). Ratchet unchanged 122/2451-equivalent (2454 with the three walk-only trivials). NEXT, in order: (1) single resolution pass at the front door (LiteralMapUnroll / ValidateDesugar consume the resolver's names; delete the second pass); (2) corpus-library.pure INTO the platform (it defines meta::pure::functions:: stdlib as harness Pure — the namespace guard must then cover every source); (3) the three harness body scans (carriesInlineCsv / callsAssert / isQuery) become platform-stated facts; (4) the referee's golden rows as a typed relation in the session, platform equality in the database, H2Verify's Java comparison deleted; (5) the main-side censuses (static sinks) deleted; THEN single-shot.

Status: **batch 114 / execution context as a VALUE LANDED (2026-09-06)** — ExecutionContext bound once by the special forms (from / execute / executionPlan / plan-execute / executeLegendQuery), read everywhere as fields; setups established once per statement; the two batch-113 route regressions closed (m2m2r::executeProjectWithNestedDerivedProperty, paginate::testPaginated). Ratchet 124/2449 → 122/2451; minimal harness 2454. Two shrink-only ratchets opened: literal Pure-name checks outside PlatformTypes (73 → 0) and no runtime-shape walker outside ExecutionContext. IMPL rows 19 → 17. NEXT = step B (delete the old harness; converge the executor's call-frame route into the splice), then the single-shot design.
...
| m2m2r::executeProjectWithNestedDerivedProperty | **LANDED batch 105, reopened 113, CLOSED 114** — the plan-execute in value position routes through its executionPlan build (RoutingContext.entryCall) and the chain rides the bound ExecutionContext — the TDS-as-row-objects JSON document emitted by the database (TDS_JSON_KV); the envelope collapse sees through the TDS cast over plan-execute values |
...
**Phase 0 (measurement integrity, 2026-09-08):** batch 126 (0.1) pins the fail roster as a SET per lane (`core/src/test/resources/rcorpus/*-fail-roster.txt`); the 121 are unchanged by name. Batch 127 (0.2): whole messages (the 11 empty roster messages now carry expected/actual; the 10 bare `Assert failed` are the engine's messageless `assert(Boolean)` substring asserts), setup failure fatal, 5 inert setups pinned. Batch 128 (0.3): zero-assertion passes excluded — DuckDB 2454 → 2422 pass = 14 SKIPPED (no assertion reachable, own roster) + 18 FAIL (`verdict-gap:guard-assert-in-expression-helper`, ledger §6: a helper's guard assert the platform never adjudicates); H2 1866 → 1834 the same way. Fail rosters: DuckDB 139, H2 727. Batch 129 (0.4): DanglingStateGuardTest (readers ⟺ writers for every static slot, all modules; guards pin only what exists) — 10 dead/unwatched statics deleted, register = {ORDERED_QUERY, SORT_KEYS} → zero in 0.5; rosters unchanged. Batch 130 (0.5): ordered compare restored as `ReplayFacts` on the SPI (three referee thread-locals gone, register ZERO; 0 tests lost to order); paginate decline-before-compare costs 9 (`paginated-golden:text-differs`, ledger §7, FIX OWED in 0.5b = page-membership verdict); DuckDB 148 fail / 14 skipped / 2413 pass, H2 736 / 14 / 1825. Batch 131 (0.5b): page-membership verdict landed — the 9 back; DuckDB 139 / 14 / 2422, H2 727 / 14 / 1834; referee 1592 / 6 / 20 + 45 page-membership. Batch 132 (0.6): named declines through the listener (ceilings per reason), FAULT outcome never text, zero broad catches in the referee, 3 referee faults fixed (H2 +13), leniency ceilings; REAL DEFECT +1 (`real-defect:splitPart-missing-part`, ledger §8) → DuckDB 140 / 14 / 2421, H2 714 / 14 / 1847. REAL-DEFECT roster of record: 37 (audit) + 18 `verdict-gap:guard-assert-in-expression-helper` (§6) + 1 splitPart (§8). Batch 133 (0.7): strength census pinned monotone — DuckDB 1512 differential / 838 literal / 22 cardinality / 49 spelling; H2 1198 / 575 / 18 / 56. Batch 134 (0.8): census 2721/146/2575 re-derived + text-scan cross-check; Ignore arm deleted; setups by effect; scanRelations exclusion measured and kept (named); EVICT_SIZE exact (723 lines burned). PHASE 0 COMPLETE (batches 126–134). Batch 135 (Phase 1, the quick win): SourceSpelling pass + one-branch unions — H2 1847 → 1961 pass (fail 600); USER: no more H2 legs, optional Phase 7 for H2 parity. Batch 136 (Phase 2a): FILTER_POS / EngineTextBoundary / TextGoldens deleted, VERBATIM_EQ → Lowerer state; zero verdict change; 8 main thread-locals left. Batch 137 (Phase 2b): RawSqlBoundary.Recorder on ExecuteOptions, per-test ReplayOracle, ExecutionTrace on ExecEnv, META ledger deleted; zero verdict change; 5 main thread-locals left. Batch 138 (Phase 2d): DERIVED_DEPTH a parameter, TestResources → ExecuteOptions.resources; 3 main thread-locals left (census attribution, die with 2c). Phase 2 standing: 12 → 3; 2c (~2 batches) recommended after Phase 3. Batch 139 (Phase 3 leg 1): splitPart = Pure's split indexed, NULL past the end (semantic SqlFn.PURE_SPLIT_PART; DuckDb list encoding, H2 + engine-text = legend_h2_extension_split_part) — DuckDB 140 → 139 fail (2422 pass); md5 ×2 + adjust StrictDate render RECLASSIFIED to REVISIT after tracing (ledger §11). Batch 140 (Phase 3 leg 2): exists-with-subtype — the implicit Inheritance op for an unmapped routed target covers routed class-typed properties (first PM no longer silently wins the navigate slot) — DuckDB 139 → 138 (2423 pass), H2 600 → 599 (1962 pass), zero LOST; homework: joinStrings ×2 + firstDayOfWeek → REVISIT; 3 simple-name failures = engine-core test-model scope (USER), 1 bare-enum ref (real). Batch 141 (Phase 3 leg 3): the 18 alloy shells — their helper is a platform native (TDG carrier reads the call as syntax); one owner per FQN → SKIPPED `no assertion reachable` on both lanes: DuckDB 120 fail / 32 SKIPPED / 2423 pass, H2 581 / 32 / 1962; correction: all 5 simple-name failures are engine-core scope. Reassessment: REAL/OPEN ≈ 8 single-test legs; the rest of the roster is ENGINE-MACHINERY ~38 / TEXT ~42 / CODE-AS-DATA+OTHER-STORE ~19 / REVISIT 6 / SCOPE 5 — the under-100 path is decisions + Phase 4 TEXT. Batch 142 (Phase 3 leg 3, the real fix; USER: fix the tests): TDG natives deleted, helpers are programs, the carrier reads constructor VALUES, the guard is a verdict over the system metamodel (schema()/table() Pure accessors; store-element identity owned once) — 18 shells PASS: DuckDB 2441 pass / 120 fail / 14 SKIPPED; H2 membership rewrite through the list carriers → 1989 pass / 572 fail (+9 exists tests). Batch 143 (Phase 3 close-out, no pass change): the quantified verdict's source reduces (forAll over enumValues->filter unrolls); the 120 traced to owners (ledger §15): 3 code legs left, REVISIT 11 (incl. 1 data nondeterminism), SCOPE 5, TEXT ~27, ENGINE ~38, CODE-AS-DATA/OTHER-STORE ~19. Batch 144 (REVISIT decided, USER agreed to all 11): ACCEPTED status + per-lane accepted register with witnesses — DuckDB 2441 pass / 109 fail / 14 SKIPPED / 11 ACCEPTED; H2 1989 / 566 / 14 / 6. Batch 145 (SCOPE decided): four engine-core fixture files as named library sources; the five scope tests reach their true walls (CODE-AS-DATA 2, ENGINE-MACHINERY 3); no pass change. Batch 0.3 will move ~32 zero-assertion passes out of the pass count (an honest drop, not a regression — plan §V2).
