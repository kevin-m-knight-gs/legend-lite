...
| **ENGINE VOCABULARY, not inventions** (11) | `avg` (legacy `~groupBy` aggregate spelling), `divideRound` (engine dynaFunction), `notEqualAnsi` (engine relationalExtension), `sub` (databricks dynaFn tests), `isNumeric` (duckdb extension), `hash` (memsql dialect), `join` (the REAL relation join's name — lite carries a same-name overload shim), `lessThan`/`lessThanEqual`/`greaterThan`/`greaterThanEqual` (+4 2026-08-16: engine DynaFunc ordering-comparison names — the engine leaves these operands UNTYPED (RelationalParseTreeWalker Literal), so join/filter conditions need Any-typed shims exactly like `notEqualAnsi`; ledger cluster 18) | the audit's census only saw pure-function FQNs, not the engine's relational operation vocabulary |
| **INTERNAL DESUGAR IR** (14, incl. `tds` — the #TDS literal's desugar target) | `legacyNavigate`, `legacyAssocPredicate`, `legacyLocalProperty`, `castAsDeclared`, `typeAsDeclared`, `otherwise`, `parseDateFormat`, `convertDateFormat`, `convertDateTimeFormat`, `convertTimeZoneFormat` (the last four are arity-disambiguating renames of the engine dynaFns `parseDate`/`convertDate`/`convertDateTime`/`convertTimeZone` with a format argument), `adjustTemporal` (+1 2026-08-15: `date::adjust` semantics carrying the LEGACY-print channel mark — engine `extensionDefaults.pure` `mapToDBUnitType` prints `dateadd` units UPPERCASE while `sqlDialectTranslation` defaults print lowercase; `TemporalFrame` stamps it on milestoning window-condition dates so `EngineStyleH2` renders the channel it quotes), `memberColumn` (B3.1 2026-09-13: a routed navigation's target read per SET — the navigating class names its own property mapping's sets and Joins' columns; the Typer mints a plain read; the resolver widens the target's union arms on demand; the engine spells member ORDINALS into the navigating class instead), `unionArm` (B3.1: the marker naming the member sets a union thread holds and their gate columns; identity on the rows) | emit-site census: each has live normalizer/lowering emitters and no upstream functional counterpart; NOT user-reachable (partition below), `tuple` (+1 2026-09-12, corpus-zero cluster A: the ROW of a graph-fetch tree's leaf values — the desugar target of pure's by-tree `isDistinct(collection, tree)`, `map(e | tuple($e.a, $e.b))->isDistinct()`; upstream's `collectionExtension.pure` body for that overload is `fail('Not implemented!')` and only its generated-Java plan binding implements it (`IsDistinctFetchTreeCoder`); here a struct literal, distinct-counted as one value) |
