...
the in-memory sides, and the pipe-joined row text); disagree 0 both channels. No
verdict-side change.

**§8.0 scope-table receipt, batch 102 / L8 groupByWithWindowSubset (2026-09-06):**
exec-passing 7, text-only 11, unable-to-exec 10 unchanged; ratchet 135/2438 → 134/2439
(testGroupByWithWindowSubset — count and cell asserts over the grouped rows); disagree 0
both channels. No verdict-side change.
...
(pin 1646 holds); VerdictQueries identifies assertEqWithinTolerance by exact FQN.
AssertLedger: the batch 93/96 receipts are `revisit:<name>` buckets — traced, NOT
resolved (user 2026-09-06), never counted as engine-golden defects.

**§8.0 scope-table receipt, batch 99 / L1 chained filters in filter position
(2026-09-06):** exec-passing 7, text-only 11, unable-to-exec 10 unchanged;
ratchet 136/2437 → 135/2438 (testChainedFiltersQuery — instance count, the
legalName value, and the assertSameSQL golden replayed on H2 against our rows);
...
multiset, and the assertSameSQL golden replayed on H2 against our union rows);
disagree 0 both channels.
...
multiset over the union rows, TDSNull for the cars); disagree 0 both channels.
...
multiset over the nested-join rows); disagree 0 both channels.
...
the row-string multiset over the nested-join rows); disagree 0 both channels.
...
**§8.0 scope-table receipt, batch 84 (2026-09-06):** exec-passing 7,
text-only 12, unable-to-exec 9 unchanged; ratchet 153/2420 → 152/2421
(testTdsJoinConcatenateAndJoin — the lineage-tree ROW verdict, lineage-rows
agree 68); disagree 0 both channels.

**§8.0 scope-table receipt, batch 83 / L16 (2026-09-06):** exec-passing 7,
text-only 12, unable-to-exec 9 unchanged; ratchet 154/2419 → 153/2420
(testSQLComments — an assert verdict over the activity row's comment, the
regex judged in the database); disagree 0 both channels.
...
engine's alias spelling, and the rows leg — both sides under the nonExecutable
pass — agrees on zero rows); exec-passing 7, unable-to-exec 9 unchanged;
ratchet 156/2417 → 155/2418; disagree 0 both channels.

**§8.0 scope-table receipt, batch 80 / L6a (2026-09-06):** exec-passing 7,
text-only 13, unable-to-exec 9 unchanged; ratchet 157/2416 → 156/2417
(testGraphFetchWithTableMapperPostProcessor — a JSON graph verdict over the
renamed, empty differentPersonTable); disagree 0 both channels.
...
executed extent's six rows); disagree 0 both channels.

**§8.0 scope-table receipt, batch 77 / L4a (2026-09-06):** exec-passing 7,
text-only 13, unable-to-exec 9 unchanged; ratchet 160/2413 → 159/2414
(testChainedJoinsWithUnionsAndIsolationWithProjectionQueryTableFilter — a rows
verdict, ['Scott','Scott','null']); disagree 0 both channels.

**§8.0 scope-table receipt, batch 76 / L8c (2026-09-06):** exec-passing 7,
text-only 13, unable-to-exec 9 unchanged; ratchet 163/2410 → 160/2413
(iqrClassifyTest, zScoreTest, testExtendDigest_InMemory — TDS value verdicts
over an in-memory TDS the database now builds from `range->map->zip` as an
UNNEST relation of the Pair layout); disagree 0 both channels.
