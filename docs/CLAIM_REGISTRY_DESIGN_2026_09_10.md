...
**Status: DRAFT for approval (USER decision §6.1). No batch-3 code before this is approved.**
...
1. **Completeness, ratcheted.** `unclaimed = Pure.all() − claimed`. Lands **red** at the
   measured count (expected ≈ 270 overloads / 175 FQNs before any explicit claims; the 70
   off-registry implementations then claim, leaving ≈ 105 FQNs = 35 grey + 70 unimplemented
   [inference from the census, verify]). The count is a **shrink-only pin** with the list
   printed to `target/unclaimed-natives.txt`; batch 4 takes it to **0** by moving the rest to
   the prelude. `UNCLAIMED_MAX` in the test is the ratchet.
2. **Exactly one.** Multi-claimed overloads = 0 (a hard assertion from day one; the derived
   sets are disjoint today — verify at landing).
3. **Honesty.** For every `ARM` claim, the owner's source file under `core/src/main/java`
   mentions the overload's bare name (`"annualized"`) or its constant (`Pure.CAL_ANNUALIZED`).
   A grep, deliberately: it cannot prove the arm lowers the call, but it makes a claim
   naming an owner that never touches the function impossible. The 35 grey rows are
   adjudicated by this rule plus one look each — a `Typer`-only mention is *not* a claim
   (type-checking a call is not implementing it); those go to the prelude or get a real arm.
4. **`Scalars.KNOWN_ABSENT` deleted** (38/39 stale) — `familyIfPresent` becomes `family`;
   a registration that finds no catalog overload dies, as it should.
...
2. **§6.2** — the prelude carries **all** upstream natives respelled (recommended), or only
   the demanded ones.
3. **§6.5 cadence** — recommended: **per upstream minor, monthly at most**, one bump PR
