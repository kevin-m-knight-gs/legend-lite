...
| B3.5 step 1 — legacy routes as composition, THE PRIMITIVE: `route(target set's function, rows, cond)` and the several-route `legacyNavigate` overload; the checker types each route on its own rows and mints the union-row keys (routes of one shape share keys → one equality; different shapes OR); the resolver builds the routed union from the routes (`ClassSources.routedUnionSource`, memoized per step) and the navigate walks its normal path; nothing emitted yet | 2026-09-13 | 108 / 444, 0 LOST, 0 GAINED (the node is neutral until the emitter switches) | INTERNAL_DESUGAR 16 → 17 (`route`); `RoutedNavigateTest` (hand-written function-form mapping: same-shape routes → one keyed join, no OR; different shapes → their own keys, OR) | docs/GATES.md "Legacy routes as composition, step 1" |
| B3.5 step 2a — the resolver asks the STEP for its target: ONE lookup (`ClassSources.navTarget(source, class, step, head)` — a step with routes → its routed union; else the class through the set-id dispatch), the eleven target-fetch sites re-pointed as a signature change (`navTargetMaterialized` receives a resolved target, resolves nothing), the six resolver rebuild sites carry routes (`withSource`/`withPredicate`/`withSourceAndPredicate`); nothing emitted yet | 2026-09-14 | 108 / 444, 0 LOST, 0 GAINED (neutral by construction) | `RoutedNavigateTest` gains the exists path | docs/GATES.md "Legacy routes as composition, step 2a" |
