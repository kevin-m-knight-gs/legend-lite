...
| F-M | The former 1,798 now sit on ONE resolver seam (1,436 + 360 rows, all `book.desk.businessUnit.legalEntity.jurisdiction` over the 359 `book`-pinned classes): a chain that starts at a ROUTED navigation head (`book`, a class-typed Join PM slot) and continues through an ASSOCIATION hop (`desk`, cross::Book_Desk) — `Substitution.rewriteMultiHop` follows a slot head only into the target's own join slots and embedded fields ("multi-hop navigation through an embedded/slot head is not supported yet"; probe: hop1Binding=null, `desk` is not among Book's bindings). The 360 are the same seam from the filter side (`Book_Desk`'s predicate applied over a row without `DESK_ID`). | SLOT PASS-THROUGH, the twin of `AssociationJoins.embeddedPassThrough`: when the chain leaves slot territory at hop k and path[k] is an association of the slot's target class, re-root at that class on the parent's materialized row under the slot's prefix (ClassSource with `composedPrefix` = the slot prefix, bindings rewritten through it) and let `registerAssociationJoins` register the remaining hops as ordinary association joins (the composed-prefix rule at StoreResolver ~1043 already re-points a head hop's condition reads). Design only; not started. | stress rows (DS* families) | open, designed |
