...
| F-P | What remains (DuckDB 156 rows / H2 211): `orElse` unported (73); a runtime binding one class through several mappings, class-query dispatch needs exactly one (15); aggregate over navigation without equi-join parent keys (5); Float-typed derived property typed Number through `trustOne` (4); two scalar-subquery fan-outs; ~45 single-row answer differences to read one by one (Measure/Unit rows, hierarchy Profile typing, a few oracle rows); H2 adds the EPOCH_MS/REVERSE walls and last-digit float / timestamp-text rows | ordered by rows: `orElse` first (one function port) | stress rows | open |
