...
| F-O | 360 rows remain from the same family on the FILTER side (DSTree*: graphFetch `book { desk { businessUnit { legalEntity { leId }}}}`): `Book_Desk`'s predicate is applied over a row without `DESK_ID` ("filter predicate references column DESK_ID, unresolvable even after isolation [param=srcRow]") — the graph-fetch child materialization of a routed head, not the projection walk | probe with `-Dstress.only=stress::DSTree_AccountOpeningStage` and `LEGEND_LITE_DUMP_SQL=1`; the association condition's parent side must read the slot's prefixed columns like the projection path now does | stress rows | open |
