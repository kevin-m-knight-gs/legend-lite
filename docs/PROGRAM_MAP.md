...
| 3 — Any positions migrate | **3a+3b LANDED; the claim is PARKED, park condition NOW MET** | 3a landed 2026-08-23 (b1cfdd97): the temporal grammar joined the carrier, #A1's subsecond strip DIED. 3b landed 2026-08-24 (bc8ba85a→3b251fc9): total two-carrier readers, the lane flag, unspell-one-side equality, and the assumption audit — decodeAny does NOT gain a carrier arm (adjudicated: spelling decode is LABEL-driven only, the LITERAL arm in unwrap; wire-sniffing deleted). The remaining hetero-LITERAL claim (TO_VARIANT sites → spellings) sits implemented on `wip/slice3-claim-on-untyped-ir` (c06743fc), parked per user ruling "typed SQL IR first, claim re-lands on it" — **that IR completed 2026-08-26** (untyped/mismatch/wire ledgers all EQUALITY-0, admissible deleted), so the re-land is unblocked and should shed the park's four compensation mechanisms |
