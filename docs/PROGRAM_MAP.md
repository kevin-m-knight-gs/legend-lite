...
| **SQL IR backend-agnosticism** | **NEW 2026-08-29 (user)**; slice 1 (identifier ORIGIN) LANDED with the convergence; **slice 2 = outputs-from-projections, NEXT UP** — charter [ORIGIN_ARCHITECTURE_AUDIT_2026_08.md](ORIGIN_ARCHITECTURE_AUDIT_2026_08.md) | slice 2 deletes the audit's 5 compensations (stampJoinOrigins, name lookup, kind fallback, union blanket stamp, renderer positional pairing): a select's outputs BUILD from its projection list at construction; heals the 2 union-wrap residue rows (h2 floor 1367 -> 1369). Ordered BEFORE the §4AD conformance leg (its new join shapes must not build on the reconciliation). Later slices: purify the ANSI base of DuckDB idioms (int-divide `//`, EXCLUDE, interval spellings, carrier caps) |
