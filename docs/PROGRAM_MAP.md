...
| **H2 session convergence — batch C** | PARKED 2026-08-29 (H2Settings note) | batches A/A2/B LANDED (emitters + renderer spell engine-style; oracle AND h2 lane both green on ENGINE_CASED by probe); cutover blocked ONLY on the PCT label seam — reference-spelling must match definition-spelling per column origin (physical vs derived), the flat SQL IR doesn't tag origin yet |
| **SQL IR backend-agnosticism** | **NEW 2026-08-29 (user)** | the "ANSI base" renderer is DuckDB-leaning by admission (int-divide `//`, EXCLUDE, interval idioms, carrier caps); purify base to true ANSI, DuckDB idioms move into the DuckDb dialect — prerequisite for the standalone typed-SQL-library vision AND likely supplies the identifier-origin tagging batch C needs |
