...
| D91 | **FIXED 2026-08-27** (F10 slice 4 fold-in — ADAPTER_NECESSITY_CENSUS §4) | `EqualityKeys.resolve` armed on EVERY lane; keyed `equal()`/`contains()`/`in` claim on plain layouts via the ONE canon owner. Residue named: `eq`/keyless-`equal`/static cross-class fold stay verdict-lane (need the minted `__id`). Witnesses: InstanceIdentityTest value-position pair. |
...
| D100 | FIXED (2026-08-26) | Resolver key widened to (model's STORE declarations, connection FQN, definition) — a connection-text-only key handed unrelated models each other's tables; the H2 default-arm mem-db NAME took the widened key too (same under-keying). Stores, not whole source: the interactive HTTP flow posts /engine/sql with a model and /engine/execute with model+query as one blob (different SOURCE, same stores — a whole-source key broke the E2E workflow test; the audit repro's models differed exactly in the store's column type, which the stores key separates). Witness: `ConnectionIsolationTest` (leak isolates + same-stores/different-source shares). |
| D101 | RE-DISPOSITIONED: ADJUDICATED + already-normalized | Top-level no-NULLS-clause is ENGINE PARITY, previously worked and REVERTED with receipts (`Fold.sortNulls` C1.2 note: extensionDefaults.pure processOrderBy emits no clause; the global ASC pin broke five engine sort pins). H2's execution lane ALREADY normalizes placement-less keys to the reference default (`H2.sortKey` NULLS LAST, landed 5d0e1ccf — PREDATES the audit base). The falsifier's "H2 inverts" evidence ran the DuckDB renderer's text via raw JDBC on H2 — a cross-execution artifact; the only real route to that state is D47's dialect-guessing (chartered). SQLite residue = D15's ruling. No code change. |
