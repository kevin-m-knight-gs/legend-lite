...
| `./mvnw verify -Dledger.update` | a maintainer, after a deliberate change of behaviour or upstream release | Accepts new baselines: rewrites the committed ledgers so the diff can be reviewed as a pull request. Everything a build can derive, it derives — see §4.8. |
...
- **Accepting a baseline** (`-Dledger.update`) rewrites committed expectations. It is not
  ...
  time (§4.8). This is the human saying "yes, that change of behaviour is intended".
...
| Parser fixture adjudication | committed | **committed** | Which construct is a legal positive and which a deliberate negative is a judgement, not a derivation. |
...
5. **Should the repository carry a half-million lines of generated corpus?** §4.8 keeps it,
   for the stated reason that deriving it needs Python and ten minutes. That is the weakest
   row in the table, and the one to revisit first — either by porting the generators to
   Java, or by publishing the corpus as a versioned artifact the build resolves.
