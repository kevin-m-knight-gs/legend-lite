...
| F10 | Variant-aware byte canon: the canon consumes TYPED values with carrier decode owned by the carrier — covers the mixed-kind-collection decline AND retires the accumulating carrier-text strips (+0000, D-suffix regexes) which are the tell of canon parsing raw carrier text. NEW WITNESS (V11): the mixed-identity VARCHAR carrier errors under candidate canon casts and rides the canon-exec decline tunnel — undetectable at wrap time because OutputCol types are stamp-derived. CENSUS 2026-08-23 (per-test decline attribution now rides ChannelB.runOne): the 13 declines attribute to 11 tests — zip x3 = the Pair-of-Pairs key leaves; letFn testLetWithParam = the F13b canon-exec array witness; Any x7 = foldEmpty + mapManyToMany + removeDuplicates NonStandard(2) + removeDuplicatesBy(2) + one mixed; mixed/kind-gate = concatenateMixedType + mixedSort + removeDuplicatesMixed. CARRIER PROBE: Any rides to_json — KIND-ERASING (temporals become bare strings byte-identical to real strings; Decimal collapses to DOUBLE); engine equal('2014-01-01', %2014-01-01) is FALSE, undecidable on this wire. The leg therefore = kind-preserving variant encoding with carrier-owned decode; blast radius: elementText, Executor decodeAny, Variant.toString (27 pinned relation rows), contains-over-Any, m2m JSON | M |
