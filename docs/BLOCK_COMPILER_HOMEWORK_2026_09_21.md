...
**Views are lifted functions — stage 1 LANDED (2026-09-22).** Homework first (docs/VIEWS_COMPILED_ONCE_HOMEWORK_2026_09_22.md). USER: a View is a zero-arg relation FUNCTION, the fifth normalizer lift (E.5), not a carrier and not a store-package expander; STRICT for a view the translator cannot expand. The accessor `#>{db.view}#` types by inlining the lifted body; the plain-columns-only view type is deleted; outside-body registers shrink one row per lane (the last DuckDB split-rung firing); rosters unchanged; the ROWS assert of the ratchet test passes, its SQL-TEXT assert is stage 4. Red first chain: the stress corpus held 4 uncounted views and one invalid fixture (fixed). Chain 417 s sequential.
