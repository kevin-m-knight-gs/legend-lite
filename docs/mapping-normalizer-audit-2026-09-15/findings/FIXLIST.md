...
> **NO-OP on the repository's machine (correction on merge, 2026-09-15):** the stale checkouts
> are the auditor's (`/Users/neemsandv`); the repository's roots (`tools/oracle-roots.sh` →
> `$HOME/legend/legend-engine`) are at the pinned `230c159196d` = 4.145.0 and the acceptance
> census below is what every lane run reports. See the correction box in
> `docs/MAPPING_NORMALIZER_AUDIT_2026_09_15.md` §2. Start at P0-1.
