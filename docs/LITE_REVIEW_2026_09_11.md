...
11. OPEN (leg C) — the `TdsLegacy` forms (restrict, restrictDistinct, tdsRows, columnValues, olapGroupBy, …) have pure BODIES upstream in tds.pure; the Typer desugars them by spelling. They could be prelude functions (the prelude carries bodies) with no typer arm at all.
12. OPEN (leg C) — `StaticFold` evaluates platform functions statically through a `switch` on the bare applied name; `TestDataGenerationNatives` switches on a stripped name over its own family. Both should switch on the resolved identity.
