...
class StressTestChaotic {
    ...
    void testChaoticModel100K() throws SQLException {
        ...
        var parsed = com.legend.parser.ElementParser.parseLegendPlatform(model);
        long parseOnlyMs = (System.nanoTime() - pCold0) / 1_000_000;

        System.out.println("\n--- PARSE PROFILING ---");
        System.out.println("Parse only:  " + parseOnlyMs + " ms  (" + parsed.elements().size() + " elements)");

        long t1cold = System.nanoTime();
        var ctx = com.legend.Compiler.compileModel(model);
        long compileMs = (System.nanoTime() - t1cold) / 1_000_000;

        System.out.println("\n--- FULL COMPILE PROFILING ---");
        System.out.println("compileModel (parse + build): " + compileMs + " ms");
        System.out.println("Implied build cost:  " + Math.max(0, compileMs - parseOnlyMs) + " ms");
        ...
        long parseNs = 0, typeNs = 0, planNs = 0;
        ...
        System.out.printf("  Pipeline: parse=%dms  lower=%dms  render=%dms%n",
                parseNs / 1_000_000, typeNs / 1_000_000, planNs / 1_000_000);
