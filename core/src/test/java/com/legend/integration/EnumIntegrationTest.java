...
class EnumIntegrationTest {
    ...
    class EndToEndTests {
        ...
        void testEnumParsedAndStored() {
            ...
            ModelContext builder = com.legend.Compiler.compileModel(pureSource);

            // THEN: The enum is stored and retrievable by simple name
            var retrieved = builder.findEnum("model::OrderStatus").orElse(null);
            ...
            assertTrue(retrieved.values().contains("PENDING"));
            assertTrue(retrieved.values().contains("SHIPPED"));
            assertFalse(retrieved.values().contains("UNKNOWN"));
        }
        ...
        void testEnumRetrievableByQualifiedName() {
            ...
            var retrieved = builder.findEnum("com::example::Priority").orElse(null);
        ...
        void testMultipleEnums() {
            ...
            assertNotNull(builder.findEnum("model::Priority").orElse(null));
            assertNotNull(builder.findEnum("model::Status").orElse(null));
            ...
            assertTrue(hasEnumValue(builder, "model::Status", "ACTIVE"));
            assertTrue(hasEnumValue(builder, "model::Color", "BLUE"));
            assertFalse(hasEnumValue(builder, "model::Priority", "LOW_PRIORITY"));
        }
        ...
        void testEnumWithClass() {
            ...
            assertNotNull(builder.findEnum("model::OrderStatus").orElse(null));
            assertNotNull(builder.findClass("model::Order").orElse(null));

            // AND: The class has the expected properties
            assertNotNull(builder.findProperty("model::Order", "status").orElse(null));
            assertNotNull(builder.findProperty("model::Order", "orderId").orElse(null));

            // AND: The status property has enum type — typeName is the canonical FQN
            // (the enum was declared as `Enum model::OrderStatus`, so FQN == "model::OrderStatus")
            assertEquals("model::OrderStatus",
                    builder.findProperty("model::Order", "status").orElseThrow().type().typeName());
            assertNotNull(builder.findProperty("model::Order", "amount").orElse(null));
        }
        ...
        void testCompleteModelWithEnum() {
            ...
            assertNotNull(builder.findEnum("model::TaskStatus").orElse(null));
            assertNotNull(builder.findClass("model::Task").orElse(null));
            assertNotNull(builder.findTable("store::TaskDB", "TASKS").orElse(null));
            assertTrue(builder.findMapping("model::TaskMapping").orElseThrow()
                    .classBindings().stream()
                    .anyMatch(b -> b.classFqn().equals("model::Task")));

            // AND: Enum values are validated
            assertTrue(hasEnumValue(builder, "model::TaskStatus", "TODO"));
            assertTrue(hasEnumValue(builder, "model::TaskStatus", "IN_PROGRESS"));
            assertTrue(hasEnumValue(builder, "model::TaskStatus", "DONE"));
            assertTrue(hasEnumValue(builder, "model::TaskStatus", "BLOCKED"));
            assertFalse(hasEnumValue(builder, "model::TaskStatus", "CANCELLED"));
        }
        ...
        void testModelContextFindEnum() {
            ...
            assertTrue(hasEnumValue(context, "model::PaymentMethod", "PAYPAL"));
            assertTrue(hasEnumValue(context, "model::PaymentMethod", "CRYPTO"));
            assertFalse(hasEnumValue(context, "model::PaymentMethod", "CASH"));
        }
        ...
        void testEnumWithSingleValue() {
            ...
            var enumDef = builder.findEnum("model::SingletonStatus").orElse(null);
            assertNotNull(enumDef);
            assertEquals(1, enumDef.values().size());
            assertTrue(enumDef.values().contains("ONLY_VALUE"));
        }
    }
    ...
    private static boolean hasEnumValue(ModelContext ctx, String fqn, String value) {
        return ctx.findEnum(fqn).map(e -> e.values().contains(value)).orElse(false);
    }
}
