...
def resolvable(c: model.Corpus, cls: str) -> list[str]:
    """Properties of `cls` the oracle can compute: a plain column, a dynafunction it
    implements, or a join chain. Anything in c.unparsed is excluded."""
...
def specs(c: model.Corpus) -> list[Spec]:
    ...
    for n, cls in enumerate(executable(c)):
        ...
        chain = [p for p in usable if (cls, p) in c.chains]
        ...
        spec.projections = ([Proj(ident, [ident])]
                            + [Proj(p, [p]) for p in plain + dyn + chain])
